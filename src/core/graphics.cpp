...
void CALL HGE_Impl::Gfx_EndScene( HWND hwndusr )
{
	RECT rcSrc, rcDest;
	rcSrc.left = 0;
	rcSrc.right = 1024;
	...
	if(!pCurTarget) 
	{
		pD3DDevice->Present( &rcSrc, &rcDest, hwndusr, NULL );
		//pD3DDevice->Present( NULL, NULL, hwndusr, NULL );
	}
}
